...
func testReplMultipleReplicas(stageHarness *test_case_harness.TestCaseHarness) error {
	...
	for i, replica := range replicas {
		logger.Infof("Testing Replica %d/%d: %s", i+1, replicaCount, replica.Logger.GetLastSecondaryPrefix())

		for i := 1; i <= len(kvMap); i++ {
			logger.Infof("%s: Expecting \"SET %s %s\" to be propagated", replica.Logger.GetLastSecondaryPrefix(), kvMap[i][0], kvMap[i][1])
