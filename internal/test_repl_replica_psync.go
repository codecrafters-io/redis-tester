...
func testReplReplicaSendsPsync(stageHarness *test_case_harness.TestCaseHarness) error {
	...
	if err := receiveReplicationHandshakeTestCase.RunReplconfStep2(server, logger); err != nil {
		return err
	}

	return receiveReplicationHandshakeTestCase.RunPsyncStep(server, logger)
}
