...
func DecodeCommandsFromAppendOnlyFile(data []byte) ([]string, error) {
	...
	for reader.Len() > 0 {
		...
		args, err := aofArrayToCommandStrings(reader, v)
...
func aofArrayToCommandStrings(reader *bytes.Reader, v resp_value.Value) (string, error) {
	elems := v.Array()
	...
	for _, elem := range elems {
		...
		case resp_value.INTEGER:
			out = append(out, strconv.Itoa(elem.Integer()))
		default:
			return "", InvalidInputError{
				Reader: reader,
				Message: fmt.Sprintf(
					"AOF command array element must be encodable as a string, got %q",
					elem.Type,
				),
			}
		}
