...
func antiCheatTest(stageHarness *test_case_harness.TestCaseHarness) error {
	...
	client, err := instrumented_resp_connection.NewInstrumentedRespClient(stageHarness, "localhost:6379", "replica")
	if err != nil {
	...
	commandTestCase := test_cases.CommandTestCase{
		Command:                   "MEMORY",
		Args:                      []string{"DOCTOR"},
		Assertion:                 resp_assertions.NewRegexStringAssertion("[\\s\\S]*[sS]am[\\s\\S]*"),
