...
func TestStages(t *testing.T) {
	...
	testCases := map[string]tester_utils_testing.TesterOutputTestCase{
		...
		"aof_pass": {
			StageSlugs:          []string{"uj3", "vd9", "fm0", "dw4", "pb9", "dc8", "fi1", "ep6", "xz2", "kn2"},
			CodePath:            "./test_helpers/pass_all",
			ExpectedExitCode:    0,
		...
		"aof_non_empty_append_only_file": {
			StageSlugs:          []string{"dw4"},
			CodePath:            "./test_helpers/scenarios/aof_non_empty_append_only_file",
			ExpectedExitCode:    1,
			StdoutFixturePath:   "./test_helpers/fixtures/aof/aof_non_empty_append_only_file",
			NormalizeOutputFunc: normalizeTesterOutput,
		},
...
func normalizeTesterOutput(testerOutput []byte) []byte {
	...
	// YG4 GET-after-propagation: CI (bare metal) often receives the value on the
	// first attempt; Docker recordings often see a nil then retry. Strip the
	// retry cycle so both timings compare equal.
